...
void loop() {
  ...
  if (state == RUMBA) error = R-(L*1.4);//M + R - 255 + 7 * L;
  ...
  switch (state) {
    ...
    // -------------------------------------------
     case START_STRAIGHT:
      ...
      if (t > 1240) advanceState(FIRST_CURVE);
      break;
    ...
    case FIRST_CURVE:
      ...
      if (((abs(error) > 70) || isLineLost(L, M, R)) && t - stateStart > 2000)  advanceState(FIRST_TURN);
      break;
    ...
    case FIRST_TURN:
      ...
      kp = 25.0; ki = 0.0005; kd = 15.0;  
      
      if ((t-stateStart > 0) && (t - stateStart < 1400)){

        digitalWrite(ALERT, HIGH);
        
        diffDrive(200, (turn*1.2) + 100);}
      ...
      // }
      if (((abs(error) > 70) || isLineLost(L, M, R)) && t - stateStart > 2000) advanceState(SECOND_TURN);
      break;
    ...
    case RUMBA:
      ...
      //if (t - stateStart > 2760) advanceState(ONE_EIGHTY);
      break;
    ...
    case ONE_EIGHTY:
      ...
      if (t - stateStart > 1500) advanceState(SHARP_SQUIGGLES);
      break;
    ...
    case SHARP_SQUIGGLES:
      ...
      if (t - stateStart > 200 && !isLineLost(L,M,R))
      {advanceState(SHARP_SQUIGGLES_2);}


      break;

    // ---------------------------------------------
    case SHARP_SQUIGGLES_2:
      digitalWrite(GREEN, LOW);
      digitalWrite(YELLOW, LOW);
      ...
      if (t - stateStart < 200) {
        diffDrive(200, turn * 1.9 + 60);
        digitalWrite(ALERT, LOW); }


      else 
        {diffDrive (200, turn *.9);
        digitalWrite(ALERT, HIGH);}
      ...
      if ((gapCounter > 400) && (t - stateStart) > 900) {
        digitalWrite(ALERT, HIGH);
    ...
    case THREE_SIXTY:
      ...
      digitalWrite(RED, LOW);
      diffDrive(255, 190);
      if (t - stateStart > 1440) advanceState(RHOMBUS);
      break;
    ...
    case RHOMBUS:
      ...
      // // If we have seen white for enough consecutive loops, switch state
      if ((gapCounter > 5) && (t - stateStart > 2500)) {
        digitalWrite(ALERT, HIGH);
        gapCounter = 0; // Reset for next time
        advanceState(CIRCLE);}
      // }
      if (t - stateStart > 2710) advanceState(CIRCLE);
      //2510
      break;
    ...
    case CIRCLE:
      ...
      digitalWrite(YELLOW, LOW);
      digitalWrite(RED, HIGH);
      if (t - stateStart < 220) diffDrive(0, -180);
      ...
      if (t - stateStart > 1850) advanceState(DONE);
      break;
